app.init_editor = function (ctl, $http, $scope, $compile, $sce) {
  ...
  ctl.read_config = function (config, str, level) {
    ...
    if (typeof config == "object" && config.length !== undefined) {
      ...
      if (eval(str).length == 0) {
        // if no elements in array, place an add button

        ctl.new_ret_stuff +=
          "<button class='btn btn-secondary' style='margin:5px 15px 5px 15px;' ng-click='ctl.add_item(" +
          str +
          ", " +
          JSON.stringify(config) +
          ", " +
          ctl.id_num +
          ")'>add new item</button><br>";
      }
    ...
    for (var index in config_keys) {
      ...
      if (config[config_keys[index]].html_type) {
        // if a leaf node

        ctl.new_ret_stuff +=
          "<div class='container' style='padding:0px; margin:0px'><div class='row' style='margin:10px 0px'><label class='ng-binding' style='margin:5px 10px; width:200px'>" +
          real_keys[index] +
          ":</label>";
      } else if (!array_bool) {
        // if an object header
        ...
        ctl.new_ret_stuff +=
          "<div style='border: solid black 1px; border-radius: 15px; margin:10px 0px 10px 0px; background-color: #" +
          ctl.color_array[level % ctl.color_array.length] +
          ";'>";

        ctl.new_ret_stuff +=
          "<button type='button' class='collapsible' style='padding:0px 20px; margin:5px 10px 5px 10px; '>" +
          real_keys[index] +
          ": </button>";
        ...
        if (
          ctl.id_num >= ctl.location_changed + ctl.difference &&
          ctl.added_or_deleted == "added"
        ) {
          unique_id = ctl.id_num - ctl.difference;
        } else if (
          ctl.id_num >= ctl.location_changed &&
          ctl.id_num < ctl.location_changed + ctl.difference &&
          ctl.added_or_deleted == "added"
        ) {
          unique_id = null;
        ...
        if (
          ...
          ctl.new_ret_stuff +=
            "<div class='content' id='" + ctl.id_num + "'><br>";
        }
        ...
        ctl.new_ret_stuff +=
          "<div style='border: solid black 1px; border-radius: 15px; padding:20px; margin:0px 20px; background-color: #" +
          ctl.color_array[level % ctl.color_array.length] +
          "'>";
      ...
      if (config[config_keys[index]].html_type == "image") {
        ctl.new_ret_stuff +=
          "</div><div><img src='" +
          eval(new_str) +
          "' style='height:150px; margin:0px 50px;' /><br>";

        ctl.new_ret_stuff +=
          "<a style='margin:0px 20px 0px 50px'>Change Image:</a>";
        ...
        ctl.new_ret_stuff +=
          "<button ng-click=\"ctl.upload($event,'" +
          new_str.replaceAll('"', "\\'") +
          "','image')\">uplaod</button>";
        ...
        if ("step" in config[config_keys[index]]) {
          ctl.new_ret_stuff +=
            ...
            config[config_keys[index]].step +
            "' class='form-control' style='width:300px' ng-model='" +
            new_str +
            "'>";
        } else {
          ctl.new_ret_stuff +=
            "<input type='number' step='0.00000001' class='form-control' style='width:300px' ng-model='" +
            new_str +
            "'>";
        }
      } else if (config[config_keys[index]].html_type == "text") {
        ctl.new_ret_stuff +=
          "<input type='text' class='form-control' style='width:300px' ng-model='" +
          new_str +
          "'>";
      } else if (config[config_keys[index]].html_type == "file") {
        ctl.new_ret_stuff +=
          "<input type='file' image_key='" +
          new_str +
          "' ng-upload-change='fileChanged($event," +
          new_str +
          ")'>";
        ...
        ctl.new_ret_stuff +=
          "<input type='file' name='my_file'accept=\"audio/*\"/></p>";
        ...
        for (option in config[config_keys[index]].options) {
          option_list +=
            "'" +
            config[config_keys[index]].options[option].replace("'", "\\'") +
            "'";
      ...
      if ("description" in config[config_keys[index]]) {
        ctl.new_ret_stuff +=
          ...
          config[config_keys[index]].description +
          "</a><br>";
      } else {
      ...
      if (config[config_keys[index]].html_type) {
        ...
        ctl.new_ret_stuff +=
          "<button class='btn btn-secondary' style='margin:5px 15px 5px 10px;' ng-click='ctl.delete_item(" +
          str +
          "," +
          index +
          "," +
        ...
        if (index == config_keys.length - 1) {
          ctl.new_ret_stuff +=
            "<div style='border: solid black 1px; border-radius: 15px; margin:0px 20px; background-color: #" +
            ctl.color_array[level % ctl.color_array.length] +
            "'>";

          ctl.new_ret_stuff +=
            "<button class='btn btn-secondary' style='margin:5px 20px 5px 10px;' ng-click='ctl.add_item(" +
            str +
            ", " +
            JSON.stringify(config) +
            ", " +
            ctl.id_num +
            ")'>add new item</button></div><br>";
        }
      }
    }
  };
  ...
  ctl.build_empty_object_from_config = function (config, object) {
    for (var index in config) {
      ...
      } else if (config[index]["html_type"] === undefined) {
        // if the next layer is not html_type, must be another object
  ...
  ctl.upload = function (ev, key, type) {
    ...
    ctl.saveData2().then(
      function (success) {
        ...
        var keys = _key
          .replaceAll("ctl.items[ctl.current_ckid]", "")
          .split("']['");
      ...
      ;*/
        ...
        oReq.open(
          "POST",
          ctl.BASE_URL + "/" + ctl.current_ckid + "/update_image",
          true
        );
