app.init_map = function(ctl){
  ...
  ctl.onLocationFound = function(e) {
    ...
    if (ctl.currentpos_marker && ctl.currentpos_marker_radius ) { // check
  ...
  ctl.mapLoaded = function(){
    ...
    $(window).on("orientationchange",function(){ 
      ...
      setTimeout(function(){$(window).scrollTop(10);},400);
      setTimeout(function(){$(window).scrollTop(110);},500);
    });
    ctl.map.invalidateSize();
    if(ctl.currentpos){
      ctl.map.locate();
    ...
    ctl.map.on('locationerror', function(e){console.log(e.message)});
    ctl.apply();
  }
  ...
  ctl.addMarker = function(item){
    ...
    var new_marker = L.marker([item.birth_certificate.lat, item.birth_certificate.lng], {customID: item.birth_certificate.ck_id, icon: icon})
