  ...
  ctl.loadView = function(){
    ...
    switch(ctl.currTemplate){
      ...
      case "/views/fragments/near_me.html":
        if(ctl.elementi.length == 0)
          ctl.loadGroup();
        ctl.nearestPlaces();
        break;
      case "/views/fragments/new.html":
        if(ctl.elementi.length == 0)
          ctl.loadGroup(ctl.openeditor);
        ctl.spinnerHide();
        ctl.apply();
        break;
      case "/views/config.html":
        ...
        break;
      default:
        console.err("non dovresti essere qui");
      break;
    }
  };
  ...
  ctl.loadGroup = function(foo){
    ...
    function(response){
      ...
      $.each(ctl.elementi, function(el){
        if(ctl.map)
          ctl.addMarker(ctl.elementi[el]);
        ctl.items[ctl.elementi[el].birth_certificate.ck_id] = ctl.elementi[el];
      });
      if(foo) foo();
      ctl.spinnerHide();
      ctl.nearestPlaces();
